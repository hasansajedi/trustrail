...
| --- | --- | --- |
| **LLM01** Prompt injection | PI-001 through PI-017 cover direct/indirect override, jailbreak, metadata poisoning, token smuggling, tool responses, encoding, multilingual attacks, payload splitting, adversarial suffixes, extracted multimodal content, invisible Unicode channels, and cross-boundary assembly; RAG-004 preserves provenance; MEM-001 protects persistent writes; the adaptive red-team gate mutates reviewed cases | Pattern detection cannot cover every semantic attack. Require least privilege, application-assigned trust, deterministic tool authorization, trusted extraction, egress controls, approval boundaries, and application-specific red teaming |
